...
async function accountHeader(req, res, next) {
  ...
  if (!req.headers.account_id) {
    const accountIdPromise = Account.find(
      { accountName: req.user.telephoneNumber },
      { _id: 1 }
    );
    const account = await accountIdPromise;
    const accountId = account.map((account) => {
      return account._id;
    });
  ...
  if (req.headers.account_id) {
    ...
    if (!isValidObjectId(req.headers.account_id)) {
      ...
      // define promises
      const OwnershipCheckPromise = Account.find({
        _id: req.headers.account_id,
        owners: req.user.telephoneNumber,
      }).countDocuments();
      const ValidAccountCheckPromise = Account.findById(
        req.headers.account_id
      ).countDocuments();
      const accountNamePromise = Account.find(
        { _id: req.headers.account_id },
        { _id: 0, accountName: 1 }
      );
