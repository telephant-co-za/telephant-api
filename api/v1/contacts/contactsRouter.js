...
router
  .get(
    ...
    asyncHandler(async (req, res, next) => {
      ...
      try {
        const contactsPromise = Contact.find({ owner: res.locals.account_name })
          .limit(limit)
          .skip((page - 1) * limit)
          .select(" _id firstName lastName telephoneNumber email owner ");

        const contacts = await contactsPromise;
        const totalDocuments = await Contact.find({
  ...
  .get(
    ...
    asyncHandler(async (req, res, next) => {
      ...
      try {
        const returnObject = await Contact.find({
          _id: ContactID,
          owner: res.locals.account_name,
        });
        ...
        if (returnObject.length > 0) {
          ...
          const err = createError(
            404,
            "Could not find this Contact ID in your contacts."
          );
          next(err);
        }
      } catch (error) {
    ...
    asyncHandler(async (req, res, next) => {
      ...
      // Pre-checks complete... Post the request
        ...
        try {
          ...
          let message = { message: "The contact was successfully created." };
    ...
    asyncHandler(async (req, res, next) => {
      ...
      try {
        ...
        if (result.deletedCount === 1) {
          let message = { message: "Contact succesfully deleted." };
          res.locals.output = message;
          next();
        } else {
          // Just in case
          ...
          try {
            ...
            if (typeof error.errors !== "undefined" && error.errors.firstName) {
              const errStr =
                error._message +
                " : " +
                error.errors.firstName.properties.message;
              const err = createError(400, errStr);
              return next(err);
            } else {
              const err = createError(500, error);
          ...
          try {
            ...
            if (typeof error.errors !== "undefined" && error.errors.lastName) {
              const errStr =
                error._message +
                " : " +
                error.errors.lastName.properties.message;
              const err = createError(400, errStr);
              return next(err);
            } else {
              const err = createError(500, error);
        ...
        if (req.body.email || req.body.email === null) {
          try {
            ...
            if (typeof error.errors !== "undefined" && error.errors.email) {
              const errStr =
                error._message + " : " + error.errors.email.properties.message;
              const err = createError(400, errStr);
              return next(err);
            } else {
              const err = createError(500, error);
