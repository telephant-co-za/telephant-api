...
router
    ...
    asyncHandler(async (req, res, next) => {
      if (res.locals.action === "listing") {
        ...
        try {
          ...
          const totalDocuments = await Account.find({
            $or: [
        ...
        try {
          const accountPromise = Account.find(
            { _id: req.headers.account_id, owners: req.user.telephoneNumber },
            {
              _id: 1,
              accountName: 1,
              description: 1,
    ...
    asyncHandler(async (req, res, next) => {
      try {
        await Account.create({
          ...
          balance: 0.0, // Will always be zero
        ...
        if (typeof error.errors !== "undefined" && error.errors.accountName) {
          const errStr =
            error._message +
            " : " +
            error.errors.accountName.properties.message;
        ...
        if (typeof error.errors !== "undefined" && error.errors.description) {
          const errStr =
            error._message +
            " : " +
            error.errors.description.properties.message;
          const err = createError(400, errStr);
          return next(err);
        } else if (typeof error !== "undefined" && error.code === 11000) {
          const errStr =
            "This group account name is already registered.  Please choose another.";
  ...
  .delete(
    ...
    asyncHandler(async (req, res, next) => {
      const errStr =
        "The ability to delete a group account has been removed.  In future a method for achieving this will be implemented.";
  ...
  /*         if (res.locals.action === 'listing') {
        ...
        else
        {
            try {
                ...
                let message = { message : "The account was succesfully deleted."}
    ...
    asyncHandler(async (req, res, next) => {
      ...
      if (res.locals.action === "listing") {
        const err = createError(
          403,
          "It is not possible to change your own account.    Please provide a different group account in the header.  See the documentation for detail."
        );
        return next(err);
      }
      ...
      // will not be in documentation as should not be known by developers
        ...
        const err = createError(
          403,
          "The request attempts to replace values on the account that are forbidden and illegal.  See the documentation for detail."
        );
        return next(err);
      }
      // cannot change the name of a group
        ...
        const err = createError(
          403,
          "It is not possible to change the name of account group."
        );
        return next(err);
      }
      ...
      if (req.body) {
        ...
        if (req.body.owner) {
          if (req.body.owner === req.user.telephoneNumber) {
            const err = createError(
              400,
              "The user telephone number is already an owner on this account."
            );
            return next(err);
          } else {
            ...
            try {
              const check = await User.findOne({
                telephoneNumber: req.body.owner,
              }).countDocuments();

              if (check != 1) {
                const err = createError(
                  400,
                  "Not a valid user's telphone number."
                );
                return next(err);
              }
            } catch (error) {
          ...
          const err = createError(
            400,
            "No relevant fields to update were defined."
          );
          return next(err);
        }
      }
